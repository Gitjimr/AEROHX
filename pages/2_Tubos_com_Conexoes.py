  ...
  with col2:
    ...
    with st.spinner('Carregando...'):
        ...
        try:
            ...
            #exporters.export(result_teste1, "conexao_h1.stl")
            #exporters.export(result_teste2_alinhado, "conexao_h2.stl")
            #exporters.export(result_teste4_alinhado, "conexao_h4.stl")
            #exporters.export(result_teste5_alinhado, "conexao_h5.stl")

            my_bar.progress(80, text='Conexões entre colunas')
            ...
            my_bar.progress(85, text='Adição de conexões entre colunas')
            ...
            for i in range(num_cols - 1):
                ...
                if i % 2 == 0:
                    if num_rows % 2 == 0:
                        translacao_pipeline1 = cq.Vector(pitch_h_ * i, (pitch_v_ * (num_rows - 1)) * 2, length)
                        pipeline1 = pipeline_.translate(translacao_pipeline1)
                    else:
                        translacao_pipeline1 = cq.Vector(pitch_h_ * i, (pitch_v_ * (num_rows - 1)) * 2, 0)
                        pipeline1 = pipeline.rotate((0, 0, 0), (1, 0, 0), 180).translate(translacao_pipeline1)

                    modelo_final2 = modelo_final2 + pipeline1

                elif i % 2 != 0:
                    translacao_pipeline2 = cq.Vector(pitch_h_ * i, pitch_v_, length)
                    pipeline2 = pipeline.translate(translacao_pipeline2)
